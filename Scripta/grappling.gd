...
func _physics_process(delta: float) -> void:
	...
	# Move down (extend rope)
	elif Input.is_action_pressed("ui_down"):
		if distance_to_anchor >= maxRopeLength:
			return
	...
	# Move up (shorten rope)
	elif Input.is_action_pressed("ui_up"):
		if distance_to_anchor <= minRopeLength:
			return
	...
	# Interaction (leave rope) — MUST be outside movement blocks
